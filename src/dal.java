...
public class dal extends JFrame {
	...
	public dal() {
		...
		JLabel lblNumeroTelefono = new JLabel("Codigo de Albaran");
		...
		JLabel lblDatosDelCliente = new JLabel("Eliminar un Albaran");
		...
		btnAceptar.setEnabled(false);
		btnAceptar.setBounds(310, 87, 132, 23);
		...
		btnComprobar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				for (int i = 0; i < parsex.arrayalbaranes.length; i++) {
					if (cdct.equals(parsex.arrayalbaranes[i][0])) {
						ciftest = 2;
					}
				...
				if (ciftest == 0) {
					JOptionPane.showMessageDialog(null, "No existe un albaran con el codigo que intentas eliminar");
				}
				if (ciftest == 2) {
