...
public class dem extends JFrame {
	...
	public dem() {
		...
		JLabel lblNumeroTelefono = new JLabel("Codigo de Empleado");
		...
		JLabel lblDatosDelCliente = new JLabel("Eliminar un Empleado");
		...
		btnComprobar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				for (int i = 0; i < parsex.arrayempleados.length; i++) {
					if (cdct.equals(parsex.arrayempleados[i][0])) {
						ciftest = 2;
					}
				...
				if (ciftest == 0) {
					JOptionPane.showMessageDialog(null, "No existe un empleado con el codigo que intentas eliminar");
				}
				if (ciftest == 2) {
