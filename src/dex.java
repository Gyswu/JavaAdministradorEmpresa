...
public class dex extends JFrame {
	...
	public dex() {
		...
		setTitle("Eliminar una existencia");
		...
		JLabel lblNumeroTelefono = new JLabel("Codigo de Existencia");
		...
		JLabel lblDatosDelCliente = new JLabel("Eliminar existencia de la base de datos");
		...
		btnComprobar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				for (int i = 0; i < parsex.arrayexistencias.length; i++) {
					if (cdct.equals(parsex.arrayexistencias[i][0])) {
						ciftest = 2;
					}
					if (parsex.arrayexistencias[i][0] == null) {
						i = parsex.arrayexistencias.length;
					}
				}
				
				if (ciftest == 0) {
					JOptionPane.showMessageDialog(null, "No existe ninguna existencia con el codigo que intentas eliminar");
				}
				if (ciftest == 2) {
