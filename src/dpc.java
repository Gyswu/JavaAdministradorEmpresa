...
public class dpc extends JFrame {
	...
	public dpc() {
		...
		JLabel lblNumeroTelefono = new JLabel("Codigo de Pedido");
		...
		JLabel lblDatosDelCliente = new JLabel("Cancelar pedido");
		...
		JButton btnAceptar = new JButton("Cancelar Pedido");
		...
		btnAceptar.setEnabled(false);
		btnAceptar.setBounds(288, 87, 154, 23);
		...
		btnComprobar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				int ciftest = 0;
				for (int i = 0; i < parsex.arraypc.length; i++) {
					if (cdct.equals(parsex.arraypc[i][0])) {
						ciftest = 2;
					}
					if (parsex.arraypc[i][0] == null) {
						i = parsex.arraypc.length;
					}
				}
				
				if (ciftest == 0) {
					JOptionPane.showMessageDialog(null, "No existe ningun pedido con el codigo que intentas eliminar");
				}
				if (ciftest == 2) {
		...
		btnComprobar.setBounds(146, 87, 132, 23);
