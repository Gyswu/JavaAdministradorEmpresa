...
public class icli extends JFrame {
	...
	public icli() {
		...
		btnAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				sql order = new sql();
				order.addcliente(rzt, cift, numtt, cet, dirt, obt, mt);
		...
		btnComprobar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				for (int i = 0; i < parsex.arrayclientes.length; i++) {
					if (cift.equals(parsex.arrayclientes[i][2])) {
						ciftest = 1;
					}
					if (parsex.arrayclientes[i][2] == null) {
				...
				if (ciftest == 0) {
					...
					JOptionPane.showMessageDialog(null, "Ya existe un cliente con el cif que intentas introducir");
				}
