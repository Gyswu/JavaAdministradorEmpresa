...
public class vpc extends JFrame {
	...
	public vpc() throws IOException {
		...
		JLabel lblPedidoDelCliente = new JLabel("Pedido del cliente");
		...
		btnCargar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				/**
				 * Busqueda de datos en pedidos a clientes para luego consultar en el resto de arrays y presentar todos los datos
				 */
				int xxx = 0;
				double tota = 0;
				for (int i = 0; i < parsex.arraypc.length; i++) {
					if (codal.getText().equals(parsex.arraypc[i][0])) {
						for (int z = 0; z < parsex.arrayclientes.length; z++) {
							if (parsex.arraypc[i][4].equals(parsex.arrayclientes[z][0])) {
						 ...
						 */
						String productos = parsex.arraypc[i][5];
